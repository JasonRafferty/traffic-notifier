...
export function renderNoTimeMatches(container, eventsOutsideTime = []) {
  container.innerHTML = `
    ...
    ${eventsOutsideTime.length > 0 ? `
      <div class="mt-4">
        <p class="text-[10px] text-slate-600 uppercase tracking-widest font-semibold mb-2">Events on this date</p>
        ${renderEventList(eventsOutsideTime)}
      </div>` : ""}`;
...
export function renderEvents(container, items) {
  container.innerHTML = renderEventList(items);
...
function renderEventList(items) {
  ...
  return `<div class="divide-y divide-[#1E2D45]">
    ${items.map((event) => {
      ...
      const type = escapeHtml(genre || segment || "event");
      const eventUrl = event.url ? escapeAttribute(event.url) : "";
      ...
      shownCount++;
      return `
        <div class="event-card py-3 select-none ${eventUrl ? "cursor-pointer" : ""} ${shownCount === 1 ? "pt-0" : ""}">
          ...
          ${eventName ? `<p class="ml-5 mb-1.5 text-xs text-slate-300 font-medium leading-snug">${eventName}</p>` : ""}
